...
void parse_descendants(cursor& c, program& p, auto&& f_child, auto&& f_field){
    if(goto_child(c)){
        do{
...
void parse_descendants(cursor& c, program& p, F&& f_child){
    parse_descendants(c, p, f_child, [](field, handle) { throw "unreachable"; });
}
template<class F>
requires requires (F f, field fld, handle h) { f(fld, h); }
void parse_descendants(cursor& c, program& p, F&& f_field){
    parse_descendants(c, p, [](symbol, handle) { throw "unreachable"; }, f_field);
}
void parse_descendants(cursor& c, program& p) {
