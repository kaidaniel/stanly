...
namespace stanly {
namespace {
auto text(const Bindings &b, const VarRef &v) -> const Text & { return b.get(v).get<0>(); }
auto record(const Bindings &b, const VarRef &v) -> const Record & { return b.get(v).get<1>(); }
...
void transition(const LoadField &n, Bindings &b) {
  update((record(b, n.source).is_bottom() or text(b, n.field).is_bottom()) ? &Text::set_to_bottom
                                                                           : &Text::set_to_top,
...
Analysis analyse(const FirstOrderGraph &) { return Analysis{FirstOrderAnalysis{}}; }
}  // namespace stanly