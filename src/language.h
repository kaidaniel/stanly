...
using syntax = metaprogramming::rebind_t<std::variant, typename L::template typelist<Repr>>;
...
template<language L>
using printable = get_syntax<L, std::string_view>;

template<language L>
using packed = get_syntax<L, idx>;

template<language L>
using typelist = typename L::typelist;



template<language L>
using record = typename L::nodes::record;
