...
constexpr std::ostream& operator<<(std::ostream& os, const stanly::handle& h) {
  return os << h.value;
}
...
struct std::hash<stanly::handle> {
  auto operator()(const stanly::handle& handle) const {
...
struct std::formatter<stanly::handle, CharT> : std::formatter<std::string_view, CharT> {
  auto format(const stanly::handle& handle, auto& ctx) const {
