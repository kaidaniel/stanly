...
auto
transmute_syntax_container(F&& f, Container&& c){
  return ranges::views::transform(std::forward<Container>(c), 
  [&f](syntax<L, Repr> variant) { 
    return std::visit([&f, &variant]<class T>(T&& node) { 
        return metaprogramming::transmute<
            T, 
            L::template typelist<Repr>,
            L::template typelist<ReturnRepr>
        >(f, std::forward<T>(node)); }, std::forward<decltype(variant)>(variant));
  });
}
...
public:
  [[nodiscard]] decltype(auto) nodes_view(){
    return transmute_syntax_container([&](idx idx){
  ...
  GraphT(std::string_view program)
    ...
    , syntax_nodes_{ranges::views::transform(
        parse<L>(program), transmute_syntax<L, std::string_view, idx>([&](std::string_view sv){
