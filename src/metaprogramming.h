...
struct push_s;
template <class T>
using head = typename head_s<T>::type;
template <class T>
using tail = typename tail_s<T>::type;
template <class A, class T>
using push = typename push_s<A, T>::type;
template <class T>
...
struct typelist;
template <>
static constexpr bool is_empty<typelist<>> = true;
template <class Head, class... Tail>
struct head_s<typelist<Head, Tail...>> {
  using type = Head;
};
template <class Head, class... Tail>
struct tail_s<typelist<Head, Tail...>> {
  using type = typelist<Tail...>;
};
template <class A, class... Args>
struct push_s<A, typelist<Args...>> {
  using type = typelist<A, Args...>;
};
static_assert(TypeList<typelist<>>);
static_assert(TypeList<typelist<typelist<int, bool>, typelist<>>>);
static_assert(std::is_same_v<push<int, push<bool, push<float, typelist<>>>>,
...
template <>
static constexpr bool is_empty<std::index_sequence<>> = true;
...
struct push_s<std::integral_constant<unsigned long, j>, std::index_sequence<is...>> {
...
template <TypeList T>
struct nth_element_s<0, T> : head_s<T> {};
template <unsigned n, TypeList T>
...
static_assert(std::is_same_v<nth_element<2, typelist<int, bool, float>>, float>);
static_assert(
    nth_element<0,
                typelist<std::integral_constant<int, 0>, std::integral_constant<int, 1>>>::value ==
    0);
...
static_assert(
    std::is_same_v<rebind<typelist, typelist<int, double, bool>>, typelist<int, double, bool>>);

// append: {xs: [*]} (x: *) -> TypeList xs -> TypeList xs::x
template <class x, TypeList T, bool = is_empty<T>>
struct append_s;
template <class x, TypeList T>
using append = typename append_s<x, T>::type;
template <class x, TypeList T>
struct append_s<x, T, empty> : push_s<x, T> {};
template <class x, TypeList T>
struct append_s<x, T, not_empty> : push_s<head<T>, append<x, tail<T>>> {};
// optimisation for typelist: O(1) instantiations (vs. O(n))
template <class x, class... xs>
struct append_s<x, typelist<xs...>, not_empty> {
  using type = typelist<xs..., x>;
};
static_assert(std::is_same_v<append<int, typelist<>>, typelist<int>>);
static_assert(
    std::is_same_v<append<typelist<int, int>, typelist<int>>, typelist<int, typelist<int, int>>>);
static_assert(
    std::is_same_v<append<bool, typelist<int, char, double>>, typelist<int, char, double, bool>>);
...
static_assert(std::is_same_v<map<std::add_pointer, typelist<int>>, typelist<int *>>);
static_assert(std::is_same_v<map<std::add_pointer, typelist<int, bool, char>>,
                             typelist<int *, bool *, char *>>);
static_assert(std::is_same_v<map<std::decay, typelist<int &&, const int &&, const int &, int *&>>,
                             typelist<int, int, int, int *>>);

// reduce: (f: * -> * -> *) -> (i: *) -> (TypeList: L) -> *
template <template <class, class> class f, class i, TypeList T, bool = is_empty<T>>
struct reduce_s;
template <template <class, class> class f, class i, TypeList T, bool = is_empty<T>>
...
template <TypeList T>
using largest = reduce<choose_larger, head<T>, tail<T>>;
static_assert(std::is_same_v<largest<typelist<short, bool, char, double>>, double>);
static_assert(std::is_same_v<largest<typelist<short, bool, std::tuple<int, int, int>, double>>,
                             /*==*/std::tuple<int, int, int>>);

// reverse: TypeList -> typelist
template <TypeList T>
using reverse = reduce<push, typelist<>, T>;
static_assert(
    std::is_same_v<reverse<typelist<int, bool, double, char>>, typelist<char, double, bool, int>>);
...
template <class t, TypeList T, bool = is_empty<T>>
struct index_s;
