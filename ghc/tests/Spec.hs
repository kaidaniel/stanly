    ...
    hspec ⎴ do
        describe "parser" ⎴ do
            it "parses some edge cases" ⎴ do
                ...
                φ bwText (parser₁ "(if ifx then theny else (if thenx then elsey else ifz))")
                    `shouldBe` Right "(if ifx then theny else (if thenx then elsey else ifz))"
                φ bwText (parser₁ "let x = (μ f. (f x)) in z")
                    `shouldBe` Right "((\955x.z) (\956f.(f x)))"
                φ bwText (parser₁ "(mu f.((((((f a) b) c) d) e) f))")
                    `shouldBe` Right "(\956f.(f a b c d e f))"
